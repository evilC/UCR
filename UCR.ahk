	...
	_RequestBinding(hk, delta := 0){
		if (delta = 0){
			...
			if (this._InputHandler.IsBindable(hk, bo)){
				hk.value := bo
				this._InputHandler.SetBinding(hk)
			}
...
Class _InputHandler {
	RegisteredBindings := {}
	__New(){
		this._JoystickThread := AhkThread(A_ScriptDir "\JoystickWatcherThread.ahk",,1) ; Loads the AutoHotkey module and starts the script.
		While !this._JoystickThread.ahkgetvar.autoexecute_done
			Sleep 50 ; wait until variable has been set.
		;this._JoystickThread.ahkExec("JoystickWatcher.RegisterAxis(1,2,3)")
...
Class _Plugin {
	...
	AddAxis(name, ChangeValueCallback, ChangeStateCallback, aParams*){
		if (!ObjHasKey(this.Axes, name)){
			this.Axes[name] := new _Axis(this, name, ChangeValueCallback, ChangeStateCallback, aParams*)
...
class _Hotkey {
	...
	_Deserialize(obj){
		...
		UCR._InputHandler.SetBinding(this)
	}
}
...
class _Axis {
	...
	__New(parent, name, ChangeValueCallback, ChangeStateCallback, aParams*){
		...
		this.ChangeValueFn := this._ChangedValue.Bind(this)
		this.ChangeValueCallback := ChangeValueCallback
		Gui, % this.ParentPlugin.hwnd ":Add", % "DDL", % "hwndhwnd " aParams[1], % "1||2|3|4|5|6"
		this.hwnd := hwnd
		this._SetGlabel(1)
	}
	...
	value[]{
		...
		set {
			this.__value := value
			OutputDebug % "Axis " this.Name " --> Plugin"
			this.ParentPlugin._ControlChanged(this)
		}
	}
